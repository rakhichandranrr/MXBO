...
if ( ! function_exists( 'einar_core_add_product_list_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_add_product_list_shortcode( $shortcodes ) {
		$shortcodes[] = 'EinarCore_Product_List_Shortcode';

		return $shortcodes;
	}

	add_filter( 'einar_core_filter_register_shortcodes', 'einar_core_add_product_list_shortcode' );
}

if ( class_exists( 'EinarCore_List_Shortcode' ) ) {
	class EinarCore_Product_List_Shortcode extends EinarCore_List_Shortcode {
		...
		public function __construct() {
			...
			$this->set_layouts( apply_filters( 'einar_core_filter_product_list_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'einar_core_filter_product_list_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( EINAR_CORE_PLUGINS_URL_PATH . '/woocommerce/shortcodes/product-list' );
			$this->set_base( 'einar_core_product_list' );
			$this->set_name( esc_html__( 'Product List', 'einar-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays list of products', 'einar-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'filterby',
					'title'         => esc_html__( 'Filter By', 'einar-core' ),
					'options'       => array(
						''             => esc_html__( 'Default', 'einar-core' ),
						'on_sale'      => esc_html__( 'On Sale', 'einar-core' ),
						'featured'     => esc_html__( 'Featured', 'einar-core' ),
						'top_rated'    => esc_html__( 'Top Rated', 'einar-core' ),
						'best_selling' => esc_html__( 'Best Selling', 'einar-core' ),
					),
					'default_value' => '',
					'group'         => esc_html__( 'Query', 'einar-core' ),
				)
			);
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'einar_core_product_list', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			$atts['data_attr']      = einar_core_get_pagination_data( EINAR_CORE_REL_PATH, 'plugins/woocommerce/shortcodes', 'product-list', 'product', $atts );

			$atts['this_shortcode'] = $this;

			return einar_core_get_template_part( 'plugins/woocommerce/shortcodes/product-list', 'templates/content', $atts['behavior'], $atts );
		}
