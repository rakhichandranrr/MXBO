...
class EinarCore_Dashboard_Import {
	/**
	 * @var EinarCore_Dashboard_Import of current class
	 */
	...
	/**
	 * @return EinarCore_Dashboard_Import
	 */
	...
	public function __construct() {
        ...
        add_action( 'import_end', array( &$this, 'update_elementor_post_meta' ) );
	}
	...
	public function set_import_url() {
		$params = EinarCore_Dashboard::get_instance()->get_import_params();
				...
				switch ( $_POST['options']['action'] ) :
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'einar-core' ) );
						}
						break;
					case 'content':
						...
						if ( $content_start ) {
							if ( ! EinarCore_Dashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
								qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'einar-core' ) );
								exit;
							}
						}
	...
	function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message( $response['response']['message'] . ' ' . esc_html__( 'Please contact support', 'einar-core' ) );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $einar_widget_id => $einar_widget_data ) {
                if ( 'block' == $einar_widget_id ) {
                    $replace_hash    = array();
                    $replace_encoded = array();
                    foreach ( $einar_widget_data as $einar_widget_block => $block_value ) {
                        ...
                        if ( strpos( $block_value['content'], 'wp:legacy-widget' ) !== false ) {
                            ...
                            array_walk_recursive( $parsed_block_content[0]['innerBlocks'], function ( $value, $key ) use ( &$replace_encoded, &$replace_hash, $einar_widget_block ) {
                                if ( 'encoded' === $key ) {
                            ...
                            $einar_widget_data[ $einar_widget_block ]['content'] = $block_value['content'];
                        }
                    }
                }
				update_option( 'widget_' . $einar_widget_id, $einar_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'einar-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}

            $this->update_widgets_after_import( $demo );
		}
	}

	public function import_sidebars_widgets( $file ) {
		$einar_sidebars = get_option( 'sidebars_widgets' );
		unset( $einar_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$einar_sidebars = array_merge( (array) $einar_sidebars, (array) $data['sidebars'] );
			unset( $einar_sidebars['wp_inactive_widgets'] );
			$einar_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $einar_sidebars );
			$einar_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $einar_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( EINAR_CORE_OPTIONS_NAME );

		if ( $options ) {

            $this->update_3rd_party_options();

			if ( $current_options != $options ) {
				$result = update_option( EINAR_CORE_OPTIONS_NAME, $options );

				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'einar-core' ) );

					$this->update_options_after_import( $file );

				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'einar-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'einar-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_einar_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'einar-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'einar-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'einar-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'einar-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'einar-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( EINAR_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$einar_import = new WP_Import();
		set_time_limit( 0 );

		$einar_import->fetch_attachments = $attachments;
		$returned_value                     = $einar_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'einar-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'einar-core' ) . ' ' . $xml );
		}
		...
		if ( $files_created ) {
			...
			if ( class_exists( 'RevSliderSliderImport' ) ) {
				...
				if ( ! empty( $import_instance ) && ! is_wp_error( $import_instance ) ) {
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								$this->set_status( 'error' );
								$this->set_message( esc_html__( 'Error while importing rev sliders', 'einar-core' ) );
								exit;
							}
						}
					}
					...
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Rev sliders imported successfully', 'einar-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'RevSliderSliderImport instance is empty', 'einar-core' ) );
				}
			} else {
				$this->set_status( 'error' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'RevSliderSliderImport class doesn\'t exist', 'einar-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'einar-core' ) );
		}
	}

    public function update_3rd_party_options() {
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( EINAR_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( EINAR_CORE_OPTIONS_NAME, $new_global_values );
		}
	}

    function update_widgets_after_import( $folder ) {
        ...
        foreach ( $demo_urls as $demo_url ) {
            ...
            $sql_query = "SELECT * FROM {$wpdb->options} WHERE option_name IN ('widget_text', 'widget_block')";
            $values    = $wpdb->get_results( $sql_query );

            if ( ! empty( $values ) ) {
                foreach ( $values as $value ) {
                    $new_value = $this->recalc_serialized_lengths( str_replace( $demo_url, $url, $value->option_value ) );
                    $wpdb->update( $wpdb->options, array( 'option_value' => $new_value ), array( 'option_id' => $value->option_id ) );
                }
            }
    ...
    function recalc_serialized_lengths( $sObject ) {
        $ret = preg_replace_callback( '/s\:(\d+)\:\"(.*?)\";/s', array(
            $this,
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = einar_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'einar-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = EinarCore_Dashboard_System_Info_Page::get_instance()->get_system_info();
    ...
    public function update_elementor_post_meta() {
        if ( qode_framework_is_installed( 'elementor' ) ) {
            $url       = esc_url( home_url( '/' ) );
            $demo_urls = $this->import_get_demo_urls( $this->folder );
...
EinarCore_Dashboard_Import::get_instance();
