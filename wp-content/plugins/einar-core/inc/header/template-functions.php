...
if ( ! function_exists( 'einar_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function einar_core_get_header_logo_image( $parameters = array() ) {
		$header_skin     = einar_core_get_post_value_through_levels( 'qodef_header_skin' );
		$hide_logo       = einar_core_get_post_value_through_levels( 'qodef_hide_logo' );
		$logo_height     = einar_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_source     = einar_core_get_post_value_through_levels( 'qodef_logo_source' );
		$logo_svg_path   = einar_core_get_post_value_through_levels( 'qodef_logo_svg_path' );
		$logo_text       = einar_core_get_post_value_through_levels( 'qodef_logo_text' );
		$customizer_logo = einar_core_get_customizer_logo();
		...
		if ( ! empty( $hide_logo ) && 'yes' === $hide_logo ) {
			$logo_classes[] = 'qodef-hide-logo--' . esc_attr( $hide_logo );
		}
		...
		if ( 'svg-path' === $logo_source && ! empty( $logo_svg_path ) ) {
			$logo_html['logo_main_image'] = apply_filters( 'einar_core_filter_header_logo_svg_path', $logo_svg_path, $parameters );

			$is_enabled = true;
		} elseif ( 'textual' === $logo_source && ! empty( $logo_text ) ) {
			$logo_html['logo_main_image'] = esc_html( apply_filters( 'einar_core_filter_header_logo_textual', $logo_text, $parameters ) );

			$is_enabled = true;
		} else {
			$available_logos = apply_filters(
				'einar_core_filter_available_header_logo_images',
				array(
					'main'  => 'main',
					'dark'  => 'dark',
					'light' => 'light',
			...
			foreach ( $available_logos as $logo_key => $option_value ) {
				...
				$logo_image_id = einar_core_get_post_value_through_levels( 'qodef_logo_' . $option_value );

				if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
					$logo_image_id = einar_core_get_post_value_through_levels( 'qodef_logo_main' );
				}
				...
				if ( ! empty( $logo_image_id ) ) {
					$logo_image_attr = array(
						...
						'itemprop' => 'image',
						'alt'      => sprintf( esc_attr__( 'logo %s', 'einar-core' ), str_replace( '_', ' ', $logo_key ) ),
					);
		...
		$parameters['logo_image'] = implode( '', apply_filters( 'einar_core_filter_header_logo_image_html', $logo_html, $parameters ) );

		if ( $is_enabled ) {
			echo apply_filters( 'einar_core_filter_get_header_logo_image', einar_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
		}
	}
}

if ( ! function_exists( 'einar_core_set_header_logo_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function einar_core_set_header_logo_styles( $style ) {
		$logo_styles  = array();
		$logo_height  = einar_core_get_post_value_through_levels( 'qodef_logo_height' );
		$logo_padding = einar_core_get_post_value_through_levels( 'qodef_logo_padding' );
		...
		// Logo SVG Source
		$svg_styles     = array();
		$svg_icon_color = einar_core_get_post_value_through_levels( 'qodef_logo_svg_path_color' );
		...
		$svg_icon_size   = einar_core_get_post_value_through_levels( 'qodef_logo_svg_path_size' );
		...
		$svg_icon_hover_color = einar_core_get_post_value_through_levels( 'qodef_logo_svg_path_hover_color' );
		...
		// Logo Textual Source
		$textual_styles       = einar_core_get_typography_styles( 'qodef_logo_text', '', null );
		$textual_hover_styles = einar_core_get_typography_hover_styles( 'qodef_logo_text', null );
	...
	add_filter( 'einar_filter_add_inline_style', 'einar_core_set_header_logo_styles' );
}

if ( ! function_exists( 'einar_core_get_header_widget_area' ) ) {
	/**
	 ...
	 * @param bool $enabled
	 */
	function einar_core_get_header_widget_area( $widget_area = 'one', $widget_area_slug = 'header-widget-area', $custom_widget_area_slug = '', $enabled = false ) {
		...
		if ( $is_enabled || $enabled ) {
			...
			$parameters = apply_filters(
				'einar_core_filter_header_widget_area',
				array(
					'page_id'             => $page_id,
			...
			einar_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}
