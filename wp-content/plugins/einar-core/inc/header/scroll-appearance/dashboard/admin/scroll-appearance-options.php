...
if ( ! function_exists( 'einar_core_add_scroll_appearance_header_options' ) ) {
	/**
	 * Function that add general options for this module
	 *
	 * @param object $page
	 */
	function einar_core_add_scroll_appearance_header_options( $page ) {
		...
		if ( $page ) {
			$scroll_appearance_tab = $page->add_tab_element(
				array(
					...
					'icon'  => 'fa fa-cog',
					'title' => esc_html__( 'Scroll Appearance Settings', 'einar-core' ),
				)
			);
			...
			$section->add_field_element(
				array(
					...
					'options'       => apply_filters( 'einar_core_filter_header_scroll_appearance_option', array( 'none' => esc_html__( 'None', 'einar-core' ) ) ),
					'default_value' => 'none',
				)
			);

			// Hook to include additional options after module options
			do_action( 'einar_core_action_after_header_scroll_appearance_options_map', $page, $section );
		}
	}

	add_action( 'einar_core_action_after_header_options_map', 'einar_core_add_scroll_appearance_header_options', 15 );
}
