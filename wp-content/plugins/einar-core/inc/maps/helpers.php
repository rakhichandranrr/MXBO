...
if ( ! function_exists( 'einar_core_get_google_maps_api_key' ) ) {
	/**
	 ...
	 * @return bool|string
	 */
	function einar_core_get_google_maps_api_key( $return_type = 'option' ) {
		$option = einar_core_get_option_value( 'admin', 'qodef_maps_api_key' );
...
if ( ! function_exists( 'einar_core_set_multiple_map_variables' ) ) {
	/**
	 ...
	 */
	function einar_core_set_multiple_map_variables( $query = array() ) {
		$map_variables = array();
		...
		if ( is_array( $query ) && count( $query ) ) {
			...
			if ( ! empty( $items ) ) {
				foreach ( $items as $id => $title ) {
					$map_variables['addresses'][] = einar_core_generate_map_params( $id, $query['post_type'] );
				}
			}
		}
...
if ( ! function_exists( 'einar_core_get_multiple_map' ) ) {
	/**
	 ...
	 * @return string
	 */
	function einar_core_get_multiple_map( $query = array() ) {
		$addresses = einar_core_set_multiple_map_variables( $query );
		...
		if ( einar_core_get_google_maps_api_key( 'is_enabled' ) ) {
			...
			$html = '<p id="qodef-multiple-map-notice">' . esc_html__( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Einar Options', 'einar-core' ) . '</p>';
		}

		do_action( 'einar_core_action_after_multiple_map' );
...
if ( ! function_exists( 'einar_core_marker_info_template' ) ) {
	/**
	 * Template with placeholders for marker info window
	 *
	 * uses underscore templates
	 */
	function einar_core_marker_info_template() {
		$marker_template = apply_filters(
			'einar_core_filter_marker_info_template',
	...
	add_action( 'einar_core_action_after_multiple_map', 'einar_core_marker_info_template' );
}

if ( ! function_exists( 'einar_core_marker_template' ) ) {
	/**
	 * Template with placeholders for marker
	 */
	function einar_core_marker_template() {
		$pin_icon = apply_filters(
			'einar_core_filter_marker_pin_icon',
	...
	add_action( 'einar_core_action_after_multiple_map', 'einar_core_marker_template' );
}
...
if ( ! function_exists( 'einar_core_generate_map_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_generate_map_params( $item_id, $post_type ) {
		$map_params = array();
		...
		// take marker pin
		$marker_pin = einar_core_get_svg_icon( 'pin' );

		// get address params
		$address_array = apply_filters( 'einar_core_filter_address_params', array(), $item_id, $post_type );
		...
		return apply_filters( 'einar_core_filter_map_params', $map_params, $item_id, $post_type );
	}
}
