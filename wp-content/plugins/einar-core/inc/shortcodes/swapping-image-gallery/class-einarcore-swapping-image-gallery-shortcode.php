...
if ( ! function_exists( 'einar_core_add_swapping_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_add_swapping_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'EinarCore_Swapping_Image_Gallery_Shortcode';

		return $shortcodes;
	}

	add_filter( 'einar_core_filter_register_shortcodes', 'einar_core_add_swapping_image_gallery_shortcode' );
}

if ( class_exists( 'EinarCore_Shortcode' ) ) {
	class EinarCore_Swapping_Image_Gallery_Shortcode extends EinarCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( EINAR_CORE_SHORTCODES_URL_PATH . '/swapping-image-gallery' );
			$this->set_base( 'einar_core_swapping_image_gallery' );
			$this->set_name( esc_html__( 'Swapping Image Gallery', 'einar-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds swapping image gallery holder', 'einar-core' ) );
			$this->set_scripts(
				array(
					'swiper'                 => array(
						'registered' => true,
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'info_position',
					'title'         => esc_html__( 'Info Position', 'einar-core' ),
					'options'       => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Description', 'einar-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'items'      => array(
						...
						array(
							'field_type' => 'text',
							'name'       => 'thumbnail_text',
							'title'      => esc_html__( 'Thumbnail Text', 'einar-core' ),
						),
						array(
							'field_type' => 'text',
							'name'       => 'thumbnail_text_label',
							'title'      => esc_html__( 'Thumbnail Text Label', 'einar-core' ),
						),
					),
				)
			);
		...
		public function render( $options, $content = null ) {
			...
			return einar_core_get_template_part( 'shortcodes/swapping-image-gallery', 'templates/swapping-image-gallery', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['info_position'] ) ? 'qodef-info-position--' . $atts['info_position'] : 'qodef-info-position--left';
