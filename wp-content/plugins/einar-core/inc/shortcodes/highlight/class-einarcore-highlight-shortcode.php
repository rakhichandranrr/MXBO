...
if ( ! function_exists( 'einar_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'EinarCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'einar_core_filter_register_shortcodes', 'einar_core_add_highlight_shortcode' );
}

if ( class_exists( 'EinarCore_Shortcode' ) ) {
	class EinarCore_Highlight_Shortcode extends EinarCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( EINAR_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'einar_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'einar-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'einar-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
            ...
            $this->set_option(
                array(
                    'field_type'    => 'select',
                    'name'          => 'content_align',
                    'title'         => esc_html__( 'Content Align', 'einar-core' ),
                    'description'   => esc_html__( 'Aligns both title and text.', 'einar-core' ),
                    'options'       => array(
		...
		public function render( $options, $content = null ) {
			...
			return einar_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
            ...
            $holder_classes[] = ! empty( $atts['content_align'] ) ? 'qodef-content-align--' . $atts['content_align'] : '';
