...
if ( ! function_exists( 'einar_core_add_pricing_items_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_add_pricing_items_shortcode( $shortcodes ) {
		$shortcodes[] = 'EinarCore_Pricing_Items_Shortcode';

		return $shortcodes;
	}

	add_filter( 'einar_core_filter_register_shortcodes', 'einar_core_add_pricing_items_shortcode' );
}

if ( class_exists( 'EinarCore_Shortcode' ) ) {
	class EinarCore_Pricing_Items_Shortcode extends EinarCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'einar_core_filter_pricing_items_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'einar_core_filter_pricing_items_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( EINAR_CORE_SHORTCODES_URL_PATH . '/pricing-items' );
			$this->set_base( 'einar_core_pricing_items' );
			$this->set_name( esc_html__( 'Pricing Items', 'einar-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds pricing items element', 'einar-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'featured_card',
					'title'         => esc_html__( 'Featured Card', 'einar-core' ),
            ...
            $this->set_option(
                array(
                    'field_type'    => 'text',
                    'name'          => 'period',
                    'title'         => esc_html__( 'Period', 'einar-core' ),
            ...
            $this->set_option(
                array(
                    'field_type' => 'image',
                    'name'       => 'image',
                    'title'      => esc_html__( 'Bar Code Image', 'einar-core' ),
                )
            );
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return einar_core_get_template_part( 'shortcodes/pricing-items', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

		private function get_holder_classes( $atts ) {
			$holder_classes = $this->init_holder_classes();

			$holder_classes[] = 'qodef-pricing-items';
			$holder_classes[] = ! empty( $atts['layout'] ) ? 'qodef-layout--' . $atts['layout'] : '';
			$holder_classes[] = ! empty( $atts['featured_card'] ) && 'yes' === $atts['featured_card'] ? 'qodef-status--featured' : 'qodef-status--regular';
