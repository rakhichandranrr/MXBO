...
if ( ! function_exists( 'einar_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function einar_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'EinarCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'einar_core_filter_register_shortcodes', 'einar_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'EinarCore_Shortcode' ) ) {
	class EinarCore_Icon_Shortcode extends EinarCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( EINAR_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'einar_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'einar-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'einar-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'size',
					'title'      => esc_html__( 'Size', 'einar-core' ),
					'options'    => array(
						'default' => esc_html__( 'Default', 'einar-core' ),
						'tiny'    => esc_html__( 'Tiny', 'einar-core' ),
						'small'   => esc_html__( 'Small', 'einar-core' ),
						'medium'  => esc_html__( 'Medium', 'einar-core' ),
						'large'   => esc_html__( 'Large', 'einar-core' ),
						'huge'    => esc_html__( 'Huge', 'einar-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'einar-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'       => array(
						'normal' => esc_html__( 'Normal', 'einar-core' ),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'einar-core' ),
					'group'      => esc_html__( 'Style', 'einar-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'hover_color',
					'title'      => esc_html__( 'Icon Hover Color', 'einar-core' ),
					'group'      => esc_html__( 'Style', 'einar-core' ),
				)
			);
			$this->set_option(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'einar-core' ),
					'group'      => esc_html__( 'Style', 'einar-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'einar_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return einar_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
